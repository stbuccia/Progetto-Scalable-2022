...
object BACKUP_newBasicAprioriMapReduce {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rdd_itemsets_1 = countItemsetsSize1(rdd, rdd_size, min_support)

    //var rdd_itemsets = time(generateAndCountItemset(rdd, rdd_itemsets_1, labelSet, rdd_size, min_support, 2))
    var rdd_itemsets = generateAndCountItemset(rdd, rdd_itemsets_1, labelSet, rdd_size, min_support, 2)

    var i = 3
    var stop = false
    while (i<5 && !stop) {
      //var rdd_itemsets_N = time(generateAndCountItemset(rdd, rdd_itemsets.filter(_._1.size==i-1), labelSet, rdd_size, min_support, i))
      var rdd_itemsets_N = generateAndCountItemset(rdd, rdd_itemsets.filter(_._1.size==i-1), labelSet, rdd_size, min_support, i)
    ...
    val associationRules = generateAssociationRules(rdd_itemsets, confidence, rdd)
